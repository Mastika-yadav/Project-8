...
return pd.DataFrame(columns=['property_name', 'hotel_star_rating', 'site_review_rating',
'combined_facilities'])
...
similar_hotels_df = similar_hotels_df[similar_hotels_df['hotel_star_rating'] >=
original_hotel_star_rating]
# Sort similar hotels by star rating and site review rating
similar_hotels_df = similar_hotels_df.sort_values(by=['hotel_star_rating', 'site_review_rating'],
ascending=[False, False])
# Get top N similar hotels in the target city
top_similar_hotels = similar_hotels_df.head()
return top_similar_hotels[['property_name', 'hotel_star_rating', 'site_review_rating',
'combined_facilities', 'city', 'pageurl', 'point_of_interest', 'address']]
...
filtered_df = filtered_df[filtered_df['combined_facilities'].str.contains(feat.strip(),
case=False, na=False)]
if filtered_df.empty:
return pd.DataFrame(columns=['property_name', 'hotel_star_rating', 'site_review_rating',
'combined_facilities', 'point_of_interest'])
filtered_df = filtered_df.sort_values(by='site_review_rating', ascending=False)
top_hotels = filtered_df.head()[['property_name', 'hotel_star_rating', 'site_review_rating',
'combined_facilities', 'point_of_interest', 'city', 'address']]
return top_hotels
def get_attractions(hotel_name, city):
city_hotels = df[df['city'].str.lower() == city.lower()]
hotel = city_hotels[city_hotels['property_name'].str.contains(hotel_name, case=False,
na=False)]
if not hotel.empty:
